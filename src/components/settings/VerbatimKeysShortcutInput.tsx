...
interface VerbatimKeysShortcutInputProps {
...
export const VerbatimKeysShortcutInput: React.FC<
  VerbatimKeysShortcutInputProps
> = ({
  ...
  const cancelRecording = useCallback(async () => {
    ...
    await commands.stopVerbatimKeysRecording().catch(console.error);
  ...
  // Set up event listener for Verbatim Keys events
  useEffect(() => {
    if (!isRecording) return;
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<VerbatimKeysEvent>(
        "verbatim-keys-event",
        async (event) => {
          if (cleanup) return;
    ...
    return () => {
      ...
      commands.stopVerbatimKeysRecording().catch(console.error);
    };
  }, [
    isRecording,
    shortcutId,
  ...
  const startRecording = async () => {
    ...
    try {
      await commands.startVerbatimKeysRecording(shortcutId);
      setIsRecording(true);
      setCurrentKeys("");
      currentKeysRef.current = "";
    } catch (error) {
