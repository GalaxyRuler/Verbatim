...
async changePostProcessInsecureLanSetting(enabled: boolean) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_post_process_insecure_lan_setting", { enabled }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async changeContextNearbyTextEnabledSetting(enabled: boolean) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_context_nearby_text_enabled_setting", { enabled }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async getDictionaryDiagnostics() : Promise<Result<DictionaryDiagnostics, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_dictionary_diagnostics") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async resetDictionaryDiagnostics() : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("reset_dictionary_diagnostics") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type DictionaryDiagnostics = { learned?: number; promoted?: number; reinforced?: number; routed?: number; skip_secure_field?: number; skip_secure_check_error?: number; skip_read_cap_exceeded?: number; skip_target_changed?: number; skip_no_post_paste_change?: number; skip_runtime_id?: number; since_ms?: number }
