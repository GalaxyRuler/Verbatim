...
- Verification completed: `cargo fmt --manifest-path src-tauri/Cargo.toml`, `cargo check --manifest-path src-tauri/Cargo.toml --no-default-features --quiet`, `cargo build --manifest-path src-tauri/Cargo.toml --no-default-features --quiet`, debug binary binding export from `src-tauri`, `cargo test --manifest-path src-tauri/Cargo.toml history --lib --no-default-features --no-run --quiet`, `bun run check:translations`, `bun run lint`, `bun run build`, and `git diff --check`.
...
- Verification completed: `cargo fmt --manifest-path src-tauri/Cargo.toml`, `cargo test --manifest-path src-tauri/Cargo.toml external_script_invocation --lib --no-default-features --no-run --quiet`, `cargo check --manifest-path src-tauri/Cargo.toml --no-default-features --quiet`, and `git diff --check`.
...
- Verification completed: `cargo fmt --manifest-path src-tauri/Cargo.toml`, `cargo test --manifest-path src-tauri/Cargo.toml "transform_replacement" --lib --no-default-features --no-run --quiet`, and `cargo check --manifest-path src-tauri/Cargo.toml --no-default-features --quiet`.
...
- Verification completed: `cargo fmt --manifest-path src-tauri/Cargo.toml`, `cargo test --manifest-path src-tauri/Cargo.toml "clipboard_restore" --lib --no-default-features --no-run --quiet`, and `cargo check --manifest-path src-tauri/Cargo.toml --no-default-features --quiet`.
...
- Verification completed: `cargo fmt --manifest-path src-tauri/Cargo.toml`, `cargo test --manifest-path src-tauri/Cargo.toml clipboard --lib --no-default-features --no-run --quiet`, `cargo check --manifest-path src-tauri/Cargo.toml --no-default-features --quiet`, and `git diff --check`.
...
- Modify: `src/bindings.ts` via debug build after backend command/type changes

- [x] Split settings into versioned domains: general, audio, insertion, privacy, models, post-processing, diagnostics, adaptive, shortcuts.
- [x] Add migrations with tests for existing settings files.
- [x] Keep generated binding regeneration immediately after adding backend commands or types.
- [ ] Do not mix this refactor into cancellation/insertion safety PRs.

**Progress 2026-06-22 (Migration Metadata):**

- Added current settings schema/domain version metadata to `AppSettings` for the planned domains: general, audio, insertion, privacy, models, post-processing, diagnostics, adaptive, and shortcuts.
- Settings load and write paths now normalize legacy settings that lack version metadata, bump stale schema/domain versions to the current version, add missing planned domains, and preserve future/unknown versions for forward compatibility.
- Added `src-tauri/src/settings_domains.rs` with the planned domain ID list, default domain-version map, and internal typed domain snapshots for general, audio, insertion, privacy, models, post-processing, diagnostics, adaptive, and shortcuts. The snapshots keep the flat persisted settings format for compatibility but establish explicit ownership for every current user setting field.
- Domain snapshots are now deserializable and can rebuild the flat `AppSettings` compatibility shape. A focused round-trip test serializes a domain document, deserializes it, converts it back to flat settings, and compares the resulting settings JSON with the original.
- Settings loading now routes through a compatibility parser that accepts both the existing flat `AppSettings` JSON and a domain-backed `{ settings_schema_version, domains }` document.
- Settings writes now persist the domain-backed document shape for startup normalization, reset-to-defaults recovery, generic settings writes, and recovered/corrupt settings rewrites, while the parser still preserves flat JSON compatibility for existing installs.
- Added read-only `get_app_settings_document` and `get_default_settings_document` commands that return the domain-backed settings document with frontend-safe API-key redaction. `src/bindings.ts` is manually mirrored for the new command/types until the documented debug build binding export is unblocked.
- Frontend settings reads now consume the domain document commands in the settings store, recording overlay, and i18n language sync path. A shared `settingsFromDocument` helper flattens the document at the UI boundary so existing settings panels continue to work while backend persistence and transport are domain-shaped.
- Playwright and accessibility Tauri mocks now serve the domain-backed settings document commands, so UI smoke and axe checks exercise the migrated frontend read path instead of relying on the legacy flat commands. The legacy flat read commands are retained as a deprecated compatibility surface until generated binding export is unblocked and removal can happen deliberately.
- Domain-document parsing now migrates older documents by merging missing domain fields or missing domain buckets from current defaults before deserialization. Regression tests cover existing flat settings, current domain documents, domain documents missing fields, and domain documents missing whole domains without resetting unrelated settings.
- Added settings-domain coverage tests that fail if a flat settings field is orphaned, assigned to multiple domains, or mapped to a non-planned domain.
- Packaged native smoke status now emits and validates the settings schema version and every planned domain version.
- Added focused tests for default version metadata, legacy files without metadata, stale version bumps, and preserving future or unknown domain entries.
- Added `src-tauri/src/bin/export_bindings.rs` and `bun run bindings:generate`, which export the shared Specta command/event registry through `--no-default-features`. `src/bindings.ts` was regenerated through that path, so settings document commands and all accumulated backend command/type changes are now generated instead of manually mirrored.
- `docs/BUILD.md` now documents `bun run bindings:generate` as the routine binding update path and reserves debug Tauri builds for broader app startup/bundling validation.
- Added explicit guarded settings-domain mutation paths, including a fallible variant for validation-heavy commands. Migrated privacy/history settings updates for history enabled, recordings enabled, history limit, and recording retention period; diagnostics writes for debug mode, lazy stream close, and log level; general writes for overlay position, docked pill, start-hidden, autostart, update checks, experimental mode, app language, tray icon visibility, custom words, and dictionary auto-add; shortcuts writes for push-to-talk, binding changes/resets, keyboard implementation fallback/switching, and invalid-binding resets; audio writes for feedback, feedback volume, sound theme, extra recording buffer, and mute-while-recording; adaptive writes for selected language, dictation language mode, profiles, context awareness, context nearby text, language shortlist, default profile, and word-correction threshold; insertion writes for paste delay, paste method, typing tool, external script path, clipboard handling, auto-submit, auto-submit key, and trailing-space behavior; models writes for Whisper/ORT accelerator settings and GPU device; and post-processing writes for translation, enabled state, formatting level, provider base URL, API-key placeholder, model, provider, prompts, and selected prompt. The helper rejects accidental cross-domain mutations and schema metadata changes before persisting the domain-backed settings document.
- Removed the deprecated flat `get_app_settings` and `get_default_settings` commands after confirming frontend, overlay, i18n, Playwright, and accessibility mocks consume the domain-document commands. Regenerated `src/bindings.ts` so the legacy flat command wrappers are gone, and moved the frontend's flattened `AppSettings` UI type to `src/lib/settingsDocument.ts` as a local projection over the domain document types.
- Remaining scope: do not split this already-mixed remediation branch into a cancellation/insertion-only PR.
- Verification completed: `cargo fmt --manifest-path src-tauri/Cargo.toml`, `cargo fmt --manifest-path src-tauri/Cargo.toml -- --check`, `cargo test --manifest-path src-tauri/Cargo.toml settings_version --lib --no-default-features --no-run --quiet`, `cargo test --manifest-path src-tauri/Cargo.toml "settings_store_parser|settings_domain|domain_document" --lib --no-default-features --no-run --quiet`, `cargo test --manifest-path src-tauri/Cargo.toml "settings_store_value|settings_store_parser|settings_domain|settings_version|domain_document" --lib --no-default-features --no-run --quiet`, `cargo test --manifest-path src-tauri/Cargo.toml "settings_store_parser|settings_store_value|settings_domain|settings_version|domain_document" --lib --no-default-features --no-run --quiet`, `cargo check --manifest-path src-tauri/Cargo.toml --no-default-features --quiet`, `bunx prettier --check src/bindings.ts`, `bun run lint`, `bun run build`, `bun run check:public-hygiene -- --all`, `$env:PLAYWRIGHT_USE_SYSTEM_CHROME='1'; bunx playwright test tests/app.spec.ts -g 'first-run onboarding verifies shortcut and microphone readiness|target-changed|language guard paste-error|paste failure toast' --project=chromium --timeout=60000`, `$env:PLAYWRIGHT_USE_SYSTEM_CHROME='1'; bunx playwright test tests/accessibility --project=chromium --timeout=60000`, `bun run bindings:generate`, `cargo check --manifest-path src-tauri/Cargo.toml --bin export_bindings --no-default-features --quiet`, `bunx prettier --check src/bindings.ts package.json docs/BUILD.md docs/plans/2026-06-21-audit-remediation-roadmap.md`, `bun run check:translations`, `git diff --check`, and `bun run tauri build -- --debug` attempted but blocked in the existing Windows `whisper-rs-sys` CMake/MSBuild step after the dedicated binding export path was added.
- Verification completed for domain writes: `cargo fmt --manifest-path src-tauri/Cargo.toml`, `cargo test --manifest-path src-tauri/Cargo.toml "settings_domain_mutation" --lib --no-default-features --no-run --quiet`, and `cargo check --manifest-path src-tauri/Cargo.toml --no-default-features --quiet`.
- Verification completed for the expanded domain-write migration: `cargo fmt --manifest-path src-tauri/Cargo.toml`, `cargo check --manifest-path src-tauri/Cargo.toml --no-default-features --quiet`, `cargo fmt --manifest-path src-tauri/Cargo.toml -- --check`, `bunx prettier --check docs/plans/2026-06-21-audit-remediation-roadmap.md`, and `git diff --check`.
- Verification completed for legacy flat read removal: `bun run bindings:generate`, `rg -n 'getAppSettings\(|getDefaultSettings\(|get_app_settings\)|get_default_settings\)' src/bindings.ts src-tauri/src/commands/mod.rs src-tauri/src/lib.rs` returned no matches, `cargo check --manifest-path src-tauri/Cargo.toml --no-default-features --quiet`, `bun run build`, and `bun run lint`.
- Verification blocked for the expanded domain-write migration: `cargo test --manifest-path src-tauri/Cargo.toml "settings_domain_mutation" --lib --no-default-features --no-run --quiet` timed out while linking the no-default-features test binary on this Windows host; the orphaned cargo parents were stopped and no cargo/rustc/link processes remain.
- Verification blocked for domain writes: executing `cargo test --manifest-path src-tauri/Cargo.toml "settings_domain_mutation" --lib --no-default-features --quiet` built the test binary but failed at process launch with `STATUS_ENTRYPOINT_NOT_FOUND` before the focused tests ran on this Windows machine.
