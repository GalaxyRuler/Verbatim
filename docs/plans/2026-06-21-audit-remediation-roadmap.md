...
- Ubuntu setup runs on `ubuntu-24.04`, installs WebKitGTK, appindicator, ALSA, OpenBLAS, X11, gtk-layer-shell, Vulkan, Mesa Vulkan drivers, `glslang-tools`, and the Vulkan SDK; the lane explicitly verifies `glslc` before Rust checks. macOS targets `aarch64-apple-darwin`.
...
- GitHub Actions run 28038758304 passed on 2026-06-23 at commit `edea3639` for `Windows x64 production backend`, `macOS ARM64 production backend`, and `Ubuntu x64 production backend`.
